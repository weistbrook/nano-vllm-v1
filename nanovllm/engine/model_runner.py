...
class ModelRunner:
    ...
    def run_model(self, input_ids: torch.Tensor, positions: torch.Tensor):
        context = get_context()
        # The captured CUDA graphs were recorded with the decode attention path
        # (paged KV cache, one query token per sequence), so only replay them for
        # pure-decode steps. Prefill / chunked-prefill steps have cu_seqlens with
        # query lengths > 1 and must run eagerly.
        if self.enforce_eager or input_ids.size(0) > 512 \
                or context.block_tables is None or context.max_seqlen_q > 1:
